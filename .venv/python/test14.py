...
Output: false"""
import re
s = input("enter string for checking segmented in list or not:")
l = [x for x in input("list for segmentation check:").split()]
x=0; y=0
for i in l:
    sx = ""; sx += i
    ax = re.findall(fr"{sx}", s, flags=re.IGNORECASE)
    x += 1
    if ax:
        y += 1
if len(l) == y:
